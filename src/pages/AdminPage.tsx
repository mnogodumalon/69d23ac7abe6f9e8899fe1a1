...
const ENTITY_TABS = [
  ...
  { key: 'zeitplan_&_ablauf', label: 'Zeitplan & Ablauf', pascal: 'ZeitplanAblauf' },
  { key: 'aufgaben_&_to_dos', label: 'Aufgaben & To-Dos', pascal: 'AufgabenToDos' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('hochzeitsdetails');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    ...
    'zeitplan_&_ablauf': new Set(),
    'aufgaben_&_to_dos': new Set(),
    'tischplan': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'zeitplan_&_ablauf': return (data as any).zeitplanAblauf as ZeitplanAblauf[] ?? [];
      case 'aufgaben_&_to_dos': return (data as any).aufgabenToDos as AufgabenToDos[] ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'zeitplan_&_ablauf' && fieldKey === 'location_ref') {
      const match = (lists.locationsList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.location_name ?? '—';
    }
    if (entity === 'zeitplan_&_ablauf' && fieldKey === 'zeitplan_dienstleister_ref') {
      const match = (lists.dienstleisterList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.firmenname ?? '—';
    }
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'zeitplan_&_ablauf': return ZEITPLANABLAUF_FIELDS;
      case 'aufgaben_&_to_dos': return AUFGABENTODOS_FIELDS;
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'zeitplan_&_ablauf' || dialogState?.entity === 'zeitplan_&_ablauf') && (
        <ZeitplanAblaufDialog
          open={createEntity === 'zeitplan_&_ablauf' || dialogState?.entity === 'zeitplan_&_ablauf'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'zeitplan_&_ablauf' ? handleUpdate : (fields: any) => handleCreate('zeitplan_&_ablauf', fields)}
          defaultValues={dialogState?.entity === 'zeitplan_&_ablauf' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'aufgaben_&_to_dos' || dialogState?.entity === 'aufgaben_&_to_dos') && (
        <AufgabenToDosDialog
          open={createEntity === 'aufgaben_&_to_dos' || dialogState?.entity === 'aufgaben_&_to_dos'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'aufgaben_&_to_dos' ? handleUpdate : (fields: any) => handleCreate('aufgaben_&_to_dos', fields)}
          defaultValues={dialogState?.entity === 'aufgaben_&_to_dos' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'zeitplan_&_ablauf' && (
        <ZeitplanAblaufViewDialog
          open={viewState?.entity === 'zeitplan_&_ablauf'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'zeitplan_&_ablauf', record: r }); }}
          locationsList={(data as any).locations ?? []}
          dienstleisterList={(data as any).dienstleister ?? []}
        />
      )}
      {viewState?.entity === 'aufgaben_&_to_dos' && (
        <AufgabenToDosViewDialog
          open={viewState?.entity === 'aufgaben_&_to_dos'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'aufgaben_&_to_dos', record: r }); }}
        />
      )}
